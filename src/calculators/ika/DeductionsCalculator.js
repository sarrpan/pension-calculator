...
function findMatchingThreshold(amount, thresholds = []) {
  return (
    thresholds.find((threshold) => amount >= num(threshold.min) && amount <= num(threshold.max)) ??
    null
  );
}
...
*/
function calculateNonProgressiveDeductionWithFloor(amount, thresholds = []) {
  ...
  const rate = num(threshold.rate);
  const floor = num(threshold.floor);

  if (rate <= 0) return 0;

  let deduction = safeAmount * rate;

  if (safeAmount - deduction < floor) {
    deduction = safeAmount - floor;
  }
...
function calculateMainEAS(mainGross) {
  return calculateNonProgressiveDeductionWithFloor(
    mainGross,
